...
>
	<section>
		...
		<ul>
			<li><strong>Footprint:</strong> the <code>stripe</code> package pulls in ~2MB and Node crypto shims.</li>
			<li><strong>Edge:</strong> SvelteKit routes can run on edge runtimes; <code>fetch</code> works there, the SDK does not.</li>
			<li><strong>Surface:</strong> a membership needs five endpoints. Writing them ourselves is less code than learning the SDK's typing quirks.</li>
			<li><strong>Auditability:</strong> every network call is visible in our own source — no vendored layer.</li>
		</ul>
	</section>
...
{`async function stripeCall<T>(method, path, body) {
  ...
  return json;
		...
		<Aside type="tip">
			<p>
				The key is loaded <strong>per-call</strong> from <code>settingsService.get</code>, which caches
				for 60 seconds. So rotating the key in the admin takes effect within a minute — no redeploy.
			</p>
		</Aside>
	</section>
	...
	<section>
		...
		<p>
			Stripe's API predates JSON-first design. It wants <code>application/x-www-form-urlencoded</code>
			with bracketed keys:
		</p>
		<CodeBlock lang="text">
{`line_items[0][price]=price_123&line_items[0][quantity]=1&mode=subscription`}
		...
		<p>
			Our <code>formEncode</code> recurses over objects + arrays and emits the bracketed form. It is the
			only "clever" code in the client; everything else is straight <code>fetch</code>.
		</p>
	</section>
	...
	<section>
		...
		<ul>
			...
			<li>API version is pinned in the header so Stripe can't silently break the response shape.</li>
		</ul>
		<h3>Next up</h3>
