...
>
	<section>
		<Steps>
			<li>Create a Stripe account at <a href="https://dashboard.stripe.com/register" rel="noreferrer">dashboard.stripe.com/register</a>.</li>
			<li>Top-right: toggle <strong>Test mode</strong> ON. Every screen should now show the "Test" pill.</li>
			<li>Install the CLI. On macOS: <code>brew install stripe/stripe-cli/stripe</code>.</li>
			<li>Authenticate: <code>stripe login</code> — opens a browser, pairs the CLI with your test account.</li>
			<li>Grab your test secret key from <strong>Developers → API keys</strong>. Paste into <a href="/admin/settings/integrations">/admin/settings/integrations</a>.</li>
		</Steps>
	</section>
...
> Use the secret to verify events at your webhook endpoint.`}
		...
		<Aside type="tip">
			<p>
				...
				Integrations. The <code>stripe listen</code> secret is stable across runs on the same
				machine — you set it once.
			</p>
		</Aside>
	</section>
...
{`stripe trigger checkout.session.completed`}
		...
		<p>
			You should see a <code>checkout.session.completed</code> line in the <code>stripe listen</code> tab,
			and a matching row at <a href="/admin/payments">/admin/payments</a> with a green
			"verified" indicator.
		</p>
	</section>
...
stripe payment_methods create --type card --card.token tok_visa`}
	...
	<section>
		...
		<ul>
			...
			<li>Settings integrations is where the secret key lives — never in <code>.env</code> once you're past localhost.</li>
		</ul>
		<h3>Next up</h3>
