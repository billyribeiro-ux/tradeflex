...
>
	<section>
		...
		<ul>
			<li>
				What <strong>Neon</strong> is: a serverless Postgres where branches are near-free copies of your
				database.
			</li>
			<li>Why we never develop against a shared dev database.</li>
			<li>
				How to authenticate <code>neonctl</code>, create a project, and read its connection string.
			</li>
			<li>
				How to pick between a <strong>pooled</strong> and <strong>direct</strong> connection string and
				why it matters for long-running queries like migrations.
			</li>
		</ul>
	</section>
	...
	<section>
		...
		<p>
			...
			creating a <em>branch</em> of a database is nearly instant and costs almost nothing. That lets each
			pull request get its own isolated database clone — a pattern we will use heavily in Module 12 (CI/CD).
		</p>

		<p>
			For local dev, you have two choices. Either you point your laptop at a <em>dev branch</em> on Neon
			(pro: zero local setup, same engine as prod; con: needs internet), or you run Postgres locally via
			Docker Compose (pro: fully offline; con: another daemon to manage). Default is the Neon dev branch.
			The Docker fallback is documented in the repo README.
		</p>
	</section>
	...
	<section>
		...
		<Steps>
			<li>
				...
				<CodeBlock lang="bash">$ pnpm add -g neonctl $ neonctl --version</CodeBlock>
				<p>Expected:</p>
			...
			<li>
				...
				<CodeBlock lang="bash"
					>$ neonctl projects create --name tradeflex --region-id aws-us-east-2</CodeBlock
				>
				<p>Pick the region closest to where you will host on Vercel. Expected output (trimmed):</p>
				<CodeBlock lang="text"
					>id proud-cloud-1234567 name tradeflex region aws-us-east-2 branches 1 (main) created
					2026-04-20T…</CodeBlock
				>
			</li>
			...
			<li>
				...
				<CodeBlock lang="text"
					>postgresql://tradeflex_owner:****@ep-xxx-pooler.us-east-2.aws.neon.tech/neondb?sslmode=require</CodeBlock
				>
				<p>
			...
			<li>
				...
				<CodeBlock title=".env" lang="dotenv"
...
DATABASE_URL_DIRECT="postgresql://…neon.tech/neondb?sslmode=require"`}</CodeBlock
				...
				<Aside type="danger" title="Never commit .env">
					<p>
						...
						file is the only version you commit, and it only contains <em>variable names</em> with empty
						values.
					</p>
				</Aside>
			</li>
			...
			<li>
				...
				<CodeBlock lang="bash"
					>$ pnpm dlx @neondatabase/cli@latest psql $DATABASE_URL -c "SELECT version();"</CodeBlock
				>
				<p>Expected:</p>
				<CodeBlock lang="text">
					version --------------------------------- PostgreSQL 16.x on x86_64-… (1 row)</CodeBlock
				>
			</li>
		</Steps>
	</section>
	...
	<section>
		...
		<ul>
			<li>
				Neon branches are near-free copies, so each surface (local dev, each PR, prod) gets its own.
			</li>
		...
		<ul>
			...
			<li>
				<code>.env</code> has both <code>DATABASE_URL</code> and <code>DATABASE_URL_DIRECT</code>.
			</li>
		</ul>
