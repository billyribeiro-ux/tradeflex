...
>
	<section>
		...
		<p>
			Trade Flex has exactly three legitimate ways to call server code from a page:
		</p>
		<ol>
			<li>
				<strong>Load functions</strong> — reads that run during navigation (covered on the previous
				page).
			</li>
			<li>
				<strong>Form actions</strong> — the classic SvelteKit way to handle form submissions with
				progressive-enhancement.
			</li>
			<li>
				<strong>Remote functions</strong> — typed RPC from client to server, introduced in
				SvelteKit 2. They come in three flavors: <code>query</code>, <code>form</code>, and
				<code>command</code>.
			</li>
		</ol>
		<p>
			All three paths land in a service. No route code queries the DB directly. No route code
			checks roles directly. That is the rule that keeps the service-layer invariant intact.
		</p>
		<Aside type="danger" title="No standalone REST/JSON endpoints for app state">
			<p>
				...
				they're easy to call from other origins. The only <code>+server.ts</code> files in Trade
				Flex are for third-party webhooks (Stripe, Resend) and a handful of health endpoints.
			</p>
		</Aside>
	</section>
	...
	<section>
		...
		<p>
			...
			they integrate cleanly with Superforms + Zod, and they're what every tutorial in the
			SvelteKit docs assumes.
		</p>
		...
		<p>
			...
			if validation fails, otherwise it hands the validated data to the service. Authorization lives
			inside the service — the route never checks a role.
		</p>
	</section>
	...
	<section>
		...
		<p>
			Remote functions are the right choice when the call doesn't map naturally to a form
			submission — for example, a "delete avatar" button, a "resend verification email" link, or a
			query that a component needs on mount that isn't part of the page's load. They are
			end-to-end typed: the client call-site gets the exact return type of the server function.
		</p>
		...
		<ul>
			...
			<li>
				<code>form</code> — like a form action but importable into any component. Pairs with a Zod
				schema for the body.
			</li>
			<li>
	...
	<section>
		...
		<p>
			...
			Renaming the server function fails the client build — a refactor invariant that REST
			endpoints can't offer.
		</p>
	</section>
	...
	<section>
		...
		<ul>
			...
			<li>Does a component need to invoke it independently of any page? → <strong>remote function</strong>.</li>
			<li>Is it a read that isn't part of initial page data? → <strong>remote <code>query</code></strong>.</li>
	...
	<section>
		...
		<p>
			With Module 2 landed, every later module inherits four invariants:
		</p>
		<ol>
			...
			<li>Every request has a <code>Caller</code> and a <code>requestId</code>, and produces one log line.</li>
			<li>Every read + write funnels through a service that takes the Caller and enforces its own rules.</li>
		</ol>
		<p>
	...
	<section>
		<h2>Recap</h2>
		<ul>
			<li>Form actions are the default for form submissions; remote functions are the default for everything else.</li>
			<li>No <code>+server.ts</code> routes for app state — only webhooks and health checks.</li>
			<li>Both paths land in the same service layer and inherit its authorization + audit guarantees.</li>
		</ul>
